...
import com.application.clubs.utils.TEAM_ID*/
import com.application.clubs.utils.*
...
class RepositoryPlaybookImpl private constructor() {
    suspend fun playbookRequestRetrofit(): List<DataPlaybook> {
        val playbookCalls = RetroFitInstance.getInstance().create(ClubsEndpoints::class.java)

        val workplaceRequest = WorkplaceRequest(TEAM_ID)
        val token = TOKEN

        return try {
            val response = playbookCalls.playbookPostData(workplaceRequest, token)
            response
        } catch (e: Exception) {
...
/*    private suspend fun eventsRequestRetrofit(
    ...
    companion object {
        private var INSTANCE: RepositoryPlaybookImpl? = null

        fun getInstance(): RepositoryPlaybookImpl = INSTANCE ?: kotlin.run {
            INSTANCE = RepositoryPlaybookImpl()
            INSTANCE!!
        }
    }
